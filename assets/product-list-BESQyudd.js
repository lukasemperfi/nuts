import{m as b,s as d,P as l,n as h}from"./footer-BnmDAafM.js";import{i as D,N as M}from"./navigation-DOgKByzg.js";const Z="/nuts/";function $(a){const{id:t,product_images:s,title:i,sku:e,subtitle:n,weight:C,weight_unit:m,packaging_types:w,discount_price:p,price:f,price_unit:g,product_statuses:o}=a,L=s.sort((c,u)=>c.sort_order-u.sort_order),v={regular:{name:"regular",class:""},sale:{name:"Акция",class:"product-card__status_sale"},new:{name:"Новинка",class:"product-card__status_new"}},r=document.createElement("div");return r.className="product-card",r.innerHTML=`
        ...
        <div class="product-card__middle">
            <a class="product-card__title" href="${Z}product/?id=${t}" name="product-card-title" aria-label="Перейти к товару">${i}</a>
            <div class="product-card__sku sku">
                <span class="sku__label">Арт:</span>
                <span class="sku__value">${e}</span>
            </div>
            <div class="product-card__subtitle">${n}</div>
        ...
        <div class="product-card__bottom">
            <div class="bottom__price price">
                ...
                ${p!==null?`<span class="price__current-price">
                      <span class="price-number">${p}</span>&nbsp;${g}.
                  </span>`:""}
                    <span class="${p!==null?"price__old-price":"price__current-price"}">
    ...
    `,y(r),r.querySelector(".product-card__buy-button").addEventListener("click",()=>{b.addItem(String(t))}),r}function y(a){const t=a.querySelector(".product-card-swiper");return t?D(t,{slidesPerView:1,navigation:{nextEl:a.querySelector(".product-card-swiper__next"),prevEl:a.querySelector(".product-card-swiper__prev")},modules:[M]}):null}const x=async a=>{let t=null;d.subscribe("products",s=>{if(s.status===l.SUCCEEDED){if(t&&JSON.stringify(t)===JSON.stringify(s.items))return;t=s.items,_(s.items,a)}}),d.subscribe("productFilters",async s=>{const i=s.filters,e=s.isInitialized;console.log("Filters changed, fetching products:",s),e&&await h(i)})},E=async a=>{const t=d.getState().products;let s=null;d.subscribe("products",i=>{if(i.status===l.SUCCEEDED){if(console.log("renderProductList from subscribe"),s&&JSON.stringify(s)===JSON.stringify(i.items))return;s=i.items,_(i.items,a)}}),t.items.length===0&&t.status===l.IDLE?(console.log("fetchProductList"),await h()):t.status===l.SUCCEEDED&&(console.log("renderProductList from existing state"),_(t.items,a))};function _(a,t){const s=document.querySelector(t);if(!s){console.error(`Ошибка: Основной контейнер с селектором "${t}" не найден.`);return}s.innerHTML="";const i=document.createElement("div");i.className="catalog",a.forEach(e=>{const n=$(e);i.appendChild(n)}),s.appendChild(i)}export{x as f,E as p};
