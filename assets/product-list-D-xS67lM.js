import{h as M,s as l,P as d,q as h}from"./footer-kwW1Dzn6.js";import{i as Z,N as $}from"./navigation-DOgKByzg.js";import{d as H,s as y}from"./toast-CN32rl5C.js";const A="/nuts/";function V(a){const{id:s,product_images:t,title:i,sku:r,subtitle:p,weight:C,weight_unit:m,packaging_types:w,discount_price:n,price:f,price_unit:v,product_statuses:o}=a,L=t.sort((e,u)=>e.sort_order-u.sort_order),g={regular:{name:"regular",class:""},sale:{name:"Акция",class:"product-card__status_sale"},new:{name:"Новинка",class:"product-card__status_new"}},c=document.createElement("div");c.className="product-card",c.innerHTML=`
        ${o.name!=="regular"?`<div class="product-card__status ${g[o.name].class}">${g[o.name].name}</div>`:""}
                ...
                <div class="swiper-wrapper">
                    ${L.map((e,u)=>`
                        <div class="swiper-slide">
                            <picture>
                                <source type="image/webp" srcset="${e.image_path_webp}">
                                <img class="product-card-swiper__image" src="${e.image_path_png}" loading="eager" alt="image-${u}" fetchpriority="high">
                            </picture>           
                        </div>
                    `).join("")}
                </div>
        ...
        <div class="product-card__middle">
            <a class="product-card__title" href="${A}product/?id=${s}" name="product-card-title" aria-label="Перейти к товару">${i}</a>
            <div class="product-card__sku sku">
                <span class="sku__label">Арт:</span>
                <span class="sku__value">${r}</span>
            </div>
            ...
            <div class="product-card__attributes">
                <div class="attributes__weight weight">
                    ...
                    <div class="weight__value-wrapper">
                        ...
                        <span class="weight__value">${C}${m}.</span>
                    </div>
                </div>
        ...
        <div class="product-card__bottom">
            <div class="bottom__price price">
                ...
                ${n!==null?`<span class="price__current-price">
                      <span class="price-number">${n}</span>&nbsp;${v}.
                  </span>`:""}
                    <span class="${n!==null?"price__old-price":"price__current-price"}">
                        <span class="price-number">${f}</span>&nbsp;${v}.
                    </span>
                </span>
            </div>
    ...
    `,E(c);const b=c.querySelector(".product-card__buy-button"),D=H(e=>{M.addItem(String(e)),y("Товар успешно добавлен в корзину!","success")},300);return b.addEventListener("click",()=>{D(s)}),c}function E(a){const s=a.querySelector(".product-card-swiper");return s?Z(s,{slidesPerView:1,navigation:{nextEl:a.querySelector(".product-card-swiper__next"),prevEl:a.querySelector(".product-card-swiper__prev")},modules:[$]}):null}const I=async a=>{let s=null;l.subscribe("products",t=>{if(t.status===d.SUCCEEDED){if(s&&JSON.stringify(s)===JSON.stringify(t.items))return;s=t.items,_(t.items,a)}}),l.subscribe("productFilters",async t=>{const i=t.filters;t.isInitialized&&await h(i)})},N=async a=>{const s=l.getState().products;let t=null;l.subscribe("products",i=>{if(i.status===d.SUCCEEDED){if(t&&JSON.stringify(t)===JSON.stringify(i.items))return;t=i.items,_(i.items,a)}}),s.items.length===0&&s.status===d.IDLE?await h():s.status===d.SUCCEEDED&&_(s.items,a)};function _(a,s){const t=document.querySelector(s);if(!t){console.error(`Ошибка: Основной контейнер с селектором "${s}" не найден.`);return}t.innerHTML="";const i=document.createElement("div");i.className="catalog",a.forEach(r=>{const p=V(r);i.appendChild(p)}),t.appendChild(i)}export{I as f,N as p};
